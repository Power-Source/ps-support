...
class PSource_Support_Network_FAQ_Categories extends PSource_Support_Admin_Menu {
	...
	public function add_menu() {
		...
		if ( isset( $_GET['action'] ) && isset( $_GET['category'] ) && 'edit' === $_GET['action'] ) {
			if ( $faq_category = psource_support_get_faq_category( absint( $_GET['category'] ) ) )
	...
	public function on_load() {
		...
		if ( ( $edit = isset( $_POST['submit-edit-faq-category'] ) || $add = isset( $_POST['submit-new-faq-category'] ) ) ) {
			$edit = isset( $_POST['submit-edit-faq-category'] );
			$add = isset( $_POST['submit-new-faq-category'] );
			
			if ( $edit ) {
				if ( ! psource_support_current_user_can( 'update_faq_category' ) )
					return;
			...
			elseif ( $add ) {
				if ( ! psource_support_current_user_can( 'insert_faq_category' ) )
					return;
			...
			if ( ! get_settings_errors( 'support_system_submit_category' ) ) {
				if ( $add ) {
					psource_support_insert_faq_category( $category_name );
	...
	public function render_inner_page() {
		if ( isset( $_GET['category'] ) && $_GET['action'] == 'edit' ) {
			...
			$cats_table = new PSource_Support_FAQ_Categories_Table();
